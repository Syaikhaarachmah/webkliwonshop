...
class Home_model extends CI_Model {
	...
	public function komentar_produk($id) {
		$this->db->select('*');
		$this->db->from('tb_produk');
		$this->db->join('tb_komentar', 'tb_komentar.komentar_proid = tb_produk.produk_id');
		$this->db->join('tb_users', 'tb_users.user_id = tb_komentar.komentar_userid');
		$this->db->where('produk_id', $id);
		$this->db->where('komentar_option', 0);
	...
	private function _sendemail($token) {
		...
		$this->email->from('emailanda@gmail.com', 'ViloShop');  //email anda di sini
	...
	public function simpan_komentar() {
		$data = array (
			'komentar_id'			=>   md5(microtime()),
			'komentar_proid'		=>   $this->input->post('produkid'),
			'komentar_userid'		=>   $this->session->userdata('userid'),
			'komentar_tgl'			=>   date('Y-m-d H:i:s'),
