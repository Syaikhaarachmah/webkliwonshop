...
class Admin extends CI_Controller {
	...
	public function komentar() {
		$data['title'] = 'Data Komentar Produk';
		$data['notifikasi'] = $this->Admin_model->data_notifikasi();
		$data['pesanmasuk'] = $this->Admin_model->data_notifikasi_pesan();
		$data['komentar'] = $this->Admin_model->data_komentar();
		$this->load->view('tema/admin/header', $data);
		$this->load->view('admin/komentar', $data);
		$this->load->view('tema/admin/footer');
	}

	public function moderasi_komentar() {
		$this->db->set('komentar_option', 1);
		$this->db->where('komentar_id', $this->uri->segment(3));
		$this->db->update('tb_komentar');
		$this->session->set_flashdata('flash', 'Status komentar berhasil diperbaharui');
		redirect('admin/komentar');
	}

	public function restore_komentar() {
		$this->db->set('komentar_option', 0);
		$this->db->where('komentar_id', $this->uri->segment(3));
		$this->db->update('tb_komentar');
		$this->session->set_flashdata('flash', 'Status komentar berhasil diperbaharui');
		redirect('admin/komentar');
